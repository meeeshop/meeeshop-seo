#!/usr/bin/env python3
"""
decrease_orange_farm_prices.py — Locate draft products under the 'Orange Farm Clothing' vendor
and subtract $20 from all their variant prices. Supports dry-run and reversion from backup logs.
"""
...
def fetch_draft_products_by_vendor(target_vendor: str = "Orange Farm Clothing") -> list:
    """Scan all products for the target vendor (case-insensitive)."""
    query = """
    ...
    query_str = f"vendor:'{target_vendor}'"
    ...
    print(f"Scanning products on Shopify for vendor '{target_vendor}'...")
    while has_next:
...
def perform_price_updates(products: list, dry_run: bool = False) -> bool:
    """Decrease price by $20 for all variants of target products."""
    mutation = """
    ...
    for idx, product in enumerate(products, 1):
        ...
        for var in variants:
            ...
            new_price = round(old_price - 10.00, 2)
            if new_price < 0:
                new_price = 0.00
...
def main():
    parser = argparse.ArgumentParser(description="Locate products for vendor Orange Farm Clothing and subtract $10 from variant prices")
    ...
    if not products:
        print(f"\nNo products found for vendor '{target_vendor}'. Nothing to do!")
        sys.exit(0)
        
    total_variants = sum(len(p["variants"]) for p in products)
    print(f"\nFound {total_variants} variant(s) across {len(products)} product(s) for vendor '{target_vendor}'.")
    
    if not args.dry_run:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        scratch_dir = os.path.join(os.path.dirname(scripts_dir), "scratch")
        os.makedirs(scratch_dir, exist_ok=True)
        backup_file = os.path.join(scratch_dir, f"backup_orange_farm_decrease_{timestamp}.json")
